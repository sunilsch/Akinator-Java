...
public class BinaryTree<ContentType> {
   ...
   public void speichere(Datei d) {
      if (this.getLeftTree().isEmpty() && this.getRightTree().isEmpty()) {
         d.writeLine("3" + this.getContent());
      }
      else if (this.getLeftTree().isEmpty()) {
         d.writeLine("2" + this.getContent());
         this.getRightTree().speichere(d);
      }
      else if (this.getRightTree().isEmpty()) {
         d.writeLine("1" + this.getContent());
         this.getLeftTree().speichere(d);
      }
      else {
         d.writeLine("0" + this.getContent());
         this.getLeftTree().speichere(d);
         this.getRightTree().speichere(d);
      }
   }
   ...
   public static BinaryTree<String> lade(Datei d) {
      String zeile = d.readLine();      
      if (zeile == null) {
         System.out.println("Die Datei enthaelt keinen gueltigen Baum!");
         System.exit(1);
      }
      
      BinaryTree<String> b = new BinaryTree(zeile.substring(1));      
      if (zeile.startsWith("2")) {
         BinaryTree<String> rt = lade(d);
         b.setRightTree(rt);
      }
      else if (zeile.startsWith("1")) {
         BinaryTree<String> lt = lade(d);
         b.setLeftTree(lt);
      }
      else if (zeile.startsWith("0")) { // bei Typ 3 passiert nichts
         BinaryTree<String> lt = lade(d);
         b.setLeftTree(lt);
         BinaryTree<String> rt = lade(d);
         b.setRightTree(rt);
